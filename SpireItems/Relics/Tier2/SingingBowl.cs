...
namespace SylmarDev.SpireItems
{
    public class SingingBowl
    {
        ...
        public void Init()
        {
            ...
            On.RoR2.CharacterBody.OnInventoryChanged += GenericPickupController_GrantItem;
            On.RoR2.CharacterBody.RecalculateStats += CharacterBody_RecalculateStats;

            Log.LogInfo("SingingBowl done.");
        }

        private void GenericPickupController_GrantItem(On.RoR2.CharacterBody.orig_OnInventoryChanged orig, CharacterBody body)
        {
            orig(body);
            int itemCountOfInventory = 0;
            if (body.inventory.GetItemCount(item.itemIndex) >= 1 && itemCountOfInventory )
            {
                healthAdded += 2 * body.inventory.GetItemCount(item.itemIndex);
